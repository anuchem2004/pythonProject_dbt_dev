            ...
            for each in root.findall('.//entry'):
                ...
                if each.find('type').text == 'JOB':
                    ...
                    new_filename = filename.replace("${marketingdb_dir_path}",
                                                    "C:/Users/anuch/Downloads/pentaho_jobs_and_transformations/pentaho_jobs_and_transformations").replace(
                        "\\", "/")
                    ...
                    print(output_list)
                    output_list = []
                elif each.find('type').text == 'SQL' and each.find('sql').text is not None:
                    ...
                    print(output_list)
                    output_list = []
                elif each.find('type').text == 'SQL' and each.find('sqlfilename').text is not None:
                    sql = each.find('sqlfilename').text
                    # print(name,",",type,",",sql)
                    output_list.append(name + "," + type + "," + sql)
                    write.writerow(output_list)
                    print(output_list)
                    output_list = []
                elif each.find('type').text == 'TRANS' :
                    trans = each.find('filename').text
                    # print(name,",",type,",",sql)
                    output_list.append(name + "," + type + "," + trans)
                    write.writerow(output_list)
                    print(output_list)
                    output_list = []
                else:
...
def extract_trans():
    ...
    for xml_data in all_files:
        ...
        with open(output_trans, 'a', newline='\n') as fwrite:
            write = csv.writer(fwrite)
            #output_list.append(main_job)
            output_list.append(xml_data)
            write.writerow(output_list)
