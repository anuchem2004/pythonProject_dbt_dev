...
--           Date_format(Now(), '%Y-%m-%d') AS load_dt
    ...
    FROM   {{source('hrst','wt_individual_f')}} i
           INNER JOIN {{ref('hrst','wt_individual_alterian_driver')}} iad
                   ON i.individual_sk = iad.individual_sk
           LEFT OUTER JOIN {{ref('hrst','wt_email_individual_bridge_distinct_alterian')}} eb
                        ON i.individual_sk = eb.individual_sk
           LEFT OUTER JOIN {{ref('hrst','wt_email_d')}} e
                        ON eb.email_sk = e.email_sk
           LEFT OUTER JOIN {{ref('hrst','wt_individual_flags_d')}} indf
                        ON i.individual_sk = indf.individual_sk
           LEFT OUTER JOIN {{source('hrst','wt_nielsen_dma_d niel')}}
