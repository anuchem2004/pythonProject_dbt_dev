                ...
                if type(term) is dict:
                    for operator, value in term.items():
                        if operator in self.COMPARISON_OPERATORS:
                            ...
                            final_term = "(%s)" % ','.join(["'%s'" % col for col in value]) if type(
                                value) is list else str(value)
                            query_terms.append("%s %s %s" % (column, operator, final_term)) if final_term.startswith(
                                '(') else query_terms.append("%s %s '%s'" % (column, operator, final_term))
                        else:
                            print('Invalid query parameter %s' % operator)
                else:
                    query_terms.append("%s='%s'" % (column, term))
